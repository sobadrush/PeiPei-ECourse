...
- **多頁面掃描**：自動翻頁讀取所有課程資訊，並抓取**閱讀時數與百分比**。
- **智慧掛機**：
  - 自動導航至課程頁面。
  - **自動跳過**：智慧判斷若閱讀時數已達 100%，則自動跳過該課程，節省時間。
  - 支援「暫停/繼續」功能，隨時掌控進度。
- **穩定性優化**：
  - 修正 macOS 上 ChromeDriver 異常退出的問題 (Status Code -9)。
  - 實作**優雅關閉**：點擊視窗關閉按鈕時，會安全釋放瀏覽器資源與記憶體。

### 📝 完善的日誌與資訊系統
- **雙向同步記錄**：執行紀錄同時顯示於介面並寫入 `./autoCourse_logs/autoCourse.log` 檔案。
- **即時資訊查詢**：介面右下角提供 **ℹ️ 資訊按鈕**，點擊可查看作者、版本與開發工具資訊。
